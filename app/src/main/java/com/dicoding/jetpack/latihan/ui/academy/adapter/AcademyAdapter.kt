...
class AcademyAdapter(private val activity: Activity?, private val mCourses: MutableList<CourseEntity>) : RecyclerView.Adapter<AcademyAdapter.AcademyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: AcademyViewHolder, position: Int) {
        holder.itemView.tvItemTitleAcademy.text = mCourses[position].title
        holder.itemView.tvItemDescriptionAcademy.text = mCourses[position].description
        holder.itemView.tvItemDateAcademy.text = String.format("Deadline %s", mCourses[position].deadline)
